...
Sandbox.prototype.eval = function (code) {
	var compiledCode = this.compile(code);
	this.evalCompiled(compiledCode);
};

Sandbox.prototype.evalCompiled = function (compiledCode) {
	if (!this.env) this.createEnv();

	var result = (new Function( "with(this) { " + compiledCode + "}")).call(this.env);
};
...
Sandbox.prototype.createEnv = function () {
	...
	with (this.env) {
		...
		objWhitelist(Math, ["abs","acos","asin","atan","atan2","ceil","cos","exp","floor","log","max","min","pow","random","round","sin","sqrt","tan"]);
		objWhitelistConst(Math, ["E","LN10","LN2","LOG10E","LOG2E","PI","SQRT1_2","SQRT2"]);
		Math.$constructor$ = Object;
		$Math$ = Math;

		objWhitelist(this.env, ["decodeURI","decodeURIComponent","encodeURI","encodeURIComponent","escape","isFinite","isNaN","parseFloat","parseInt","unescape"]);
