...
try {
	// Node.js
	jsp = require("../vendor/UglifyJS/lib/parse-js");
} catch (e) {
	// Browser
	// TODO: The Node.js version should work in the browser as well.
	//       This is a bug upstream in Stitch.
...
Sandbox.prototype.evalCompiled = function (code) {
	...
	if ("function" === typeof code) {
		...
		return this.env.runInContext(code, "uglybox.vm");
	} else {
		if (window.opera) {
			this.env.Function(code)();
		} else {
			this.env.document.open();
			this.env.document.write('<meta http-equiv="X-UA-Compatible" content="IE=Edge" />');
			this.env.document.write('<script type="text/javascript">' + code + '<\/script>');
			this.env.document.close();
		}
	}
};
...
Sandbox.prototype.includeCompiled = function (file) {
	...
	if (isNode) {
		return this.env.runInContext(fs.readFileSync(file), file);
	} else {
		var script = this.env.document.createElement('script');
		script.type = 'text/javascript';
		script.src = file;
		var head = this.env.document.getElementsByTagName('head')[0];
...
Sandbox.prototype.getEnv = function () {
	if (!this.env) this.createEnv();
...
Sandbox.prototype.createEnv = function () {
	...
	if (isNode) {
		this.env = vm.Script.createContext();
		this.env.console = console;
		this.env.vm = vm;
		this.env.global = this.env;
		this.env.sub = function () {};
		this.env.runInContext = function (code, filename) {
			if ("function" === typeof code) {
				//return code();
			} else {
				code = code.toString();
				var script = vm.createScript(code, filename);
				return script.runInContext(self.env);
			}
		};
		var bootstrapPath = __dirname+'/bootstrap.js';
		var script = vm.createScript(fs.readFileSync(bootstrapPath), bootstrapPath);
		script.runInContext(this.env);
		sourceScope = global;
	} else if (window && document) { // Browser
		...
		this.env = iframe.contentWindow;
		sourceScope = window;

		this.env.require = require;
		this.evalCompiled(bootstrapCode);
	} else {
		throw new Error("Unknown/unsupported type of JavaScript environment");
	}

	function objWhitelist(obj, keys, source) {
		source = source || obj;
		for (var i = 0; i < keys.length; i++) {
			var prop = keys[i];
			obj.prototype["$"+prop+"$"] = (function (obj, prop, source) {
				function func() {
					return source[prop].apply(this, arguments);
				}
	...
	objWhitelistConst(this.env, ["decodeURI","decodeURIComponent","encodeURI","encodeURIComponent","escape","isFinite","isNaN","parseFloat","parseInt","unescape"], sourceScope);

	// array/object [] accessor
	this.env.sub = function (key) {
	...
	// array constant
	this.env.arr = function () {
		return Array.prototype.slice.call(arguments, 0);
	};

	var intervalIds = [];
	var timeoutIds = [];
	this.env.$clearInterval$ = function $clearInterval$(id) {
		id = +id;
	...
	this.env.$clearTimeout$ = function $clearTimeout$(id) {
		id = +id;
	...
	this.env.$setInterval$ = function $setInterval$(func, time) {
		time = +time;
		if (time <= 0) time = null;
	...
	this.env.$setTimeout$ = function $setTimeout$(func, time) {
		time = +time;
		if (time <= 0) time = null;
	...
	this.env.$eval$ = function $eval$(code) {
		self.eval(code);
	};

	if (!this.env.console) this.env.console = console;

	this.env.handleException = function handleException(e) {
		if (e.stack) console.log(e.stack.toString());
		else console.log(e.message);
	};

	RegExp.prototype.foobar = "bla";

	this.env.$window$ = this.env;
};
...
Sandbox.prototype.genCode = function (ast, options) {
	...
	function make_name_prop(name) {
		if (HOP(ALLOWED_PROPERTIES, name)) return name;
	...
	var generators = {
		...
		"dot": function(expr) {
			...
			while (i < arguments.length)
				out += "." + make_name_prop(arguments[i++]);
			return out;
		},
